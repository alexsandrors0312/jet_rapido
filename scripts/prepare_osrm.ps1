...
docker run --rm -v $volumeMount ghcr.io/project-osrm/osrm-backend:v5.27.1 osrm-extract -p /opt/foot.lua "/data/map$extension"
if ($LASTEXITCODE -ne 0) { throw 'Falha no osrm-extract. O material foi preservado para diagnóstico.' }
docker run --rm -v $volumeMount ghcr.io/project-osrm/osrm-backend:v5.27.1 osrm-contract /data/map.osrm
